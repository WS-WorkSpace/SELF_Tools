...
@interface SELF_TabBar ()

// 对内指针
@property (nonatomic, strong) NSMutableArray <SELF_TabBarItem *> *items;


@end

@implementation SELF_TabBar
#pragma mark - 构造
- (instancetype)init{
    self = [super init];
    if (self) {
...
#pragma mark - 常规配置
// 设置items
- (void)setTabBarConfig:(NSArray<SELF_TabBarConfigModel *> *)tabBarConfig{
    _tabBarConfig = tabBarConfig;
    [_tabBarConfig enumerateObjectsUsingBlock:^(SELF_TabBarConfigModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        SELF_TabBarItem *item = [[SELF_TabBarItem alloc] initWithModel:obj]; // 模型转成实例
...
static SELF_TabBarItem *lastItem;
// 点击的tabbarItem
- (void)click_tabBarItem:(SELF_TabBarItem *)sender{
...
- (void)switch_tabBarItemIndex:(NSInteger )index WithAnimation:(BOOL )animation{
    // 1.切换tabbar的状态
    [self.items enumerateObjectsUsingBlock:^(SELF_TabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        item.isSelect = index == idx; // 当前点击的调整选中，其他否定
    }];
    // 动画逻辑
    WeakSelf; // 2.通过回调点击事件让代理去执行切换选项卡的任务
    SELF_TabBarItem *item = self.items[index];
    if (item.itemModel.isRepeatClick) { // 允许重复点击触发动画
        if (animation) [item startStrringConfigAnimation]; // 开始执行设置的动画效果
        if (self.delegate && [self.delegate respondsToSelector:@selector(SELF_TabBar:selectIndex:)]) {
            [self.delegate SELF_TabBar:weakSelf selectIndex:index];
        }
    }else{
...
- (void)viewDidLayoutItems{
    ...
    [self.items enumerateObjectsUsingBlock:^(SELF_TabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (SELF_IsiPhoneX || itemHeight > 50) {
        ...
        if (isNoSettingItemSize) { // 没设置则为默认填充模式
            ...
            switch (item.itemModel.alignmentStyle) { // item对齐模式
                case SELF_TabBarConfigAlignmentStyleCenter:{               // 居中 默认
                ...
                case SELF_TabBarConfigAlignmentStyleCenterTop:{            // 居中顶部对齐
                    itemFrame.origin.x = idx * screenAverageWidth + (screenAverageWidth - item.itemModel.itemSize.width)/2;
                    itemFrame.origin.y = 0;
                }break;
                case SELF_TabBarConfigAlignmentStyleCenterLeft:{           // 居中靠左对齐
                    itemFrame.origin.x = idx * screenAverageWidth;
                    itemFrame.origin.y = (itemHeight - item.itemModel.itemSize.height)/2;
                }break;
                case SELF_TabBarConfigAlignmentStyleCenterRight:{          // 居中靠右对齐
                    itemFrame.origin.x = idx * screenAverageWidth + (screenAverageWidth - item.itemModel.itemSize.width);
                    itemFrame.origin.y = (itemHeight - item.itemModel.itemSize.height)/2;
                }break;
                case SELF_TabBarConfigAlignmentStyleCenterBottom:{         // 居中靠下对齐
                    itemFrame.origin.x = idx * screenAverageWidth + (screenAverageWidth - item.itemModel.itemSize.width)/2;
                    itemFrame.origin.y = itemHeight - item.itemModel.itemSize.height;
                }break;
                case SELF_TabBarConfigAlignmentStyleTopLeft:{        // 上左对齐
                    itemFrame.origin.x = idx * screenAverageWidth;
                    itemFrame.origin.y = 0;
                }break;
                case SELF_TabBarConfigAlignmentStyleTopRight:{       // 上右对齐
                    itemFrame.origin.x = idx * screenAverageWidth + (screenAverageWidth - item.itemModel.itemSize.width);
                    itemFrame.origin.y = 0;
                }break;
                case SELF_TabBarConfigAlignmentStyleBottomLeft:{     // 下左对齐
                    itemFrame.origin.x = idx * screenAverageWidth;
                    itemFrame.origin.y = itemHeight - item.itemModel.itemSize.height;
                }break;
                case SELF_TabBarConfigAlignmentStyleBottomRight:{    // 下右对齐
                    itemFrame.origin.x = idx * screenAverageWidth + (screenAverageWidth - item.itemModel.itemSize.width);
                    itemFrame.origin.y = itemHeight - item.itemModel.itemSize.height;
                }break;
                default: break;
...
- (void)setBadge:(NSString *)Badge index:(NSUInteger)index{
    if (index < self.items.count) {
        ...
        NSException *excp = [NSException exceptionWithName:@"SELF_TabBar Error"
...
- (void)itemDidLayoutBulge{
    ...
    [self.items enumerateObjectsUsingBlock:^(SELF_TabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.itemModel.bulgeStyle) {
            case SELF_TabBarConfigBulgeStyleNormal:{
            }break;         // 无 默认
            case SELF_TabBarConfigBulgeStyleCircular:{
            ...
            case SELF_TabBarConfigBulgeStyleSquare:{
                itemFrame.origin.y = - item.itemModel.bulgeHeight;
...
#pragma mark - SET/GET
- (NSArray<SELF_TabBarItem *> *)tabBarItems{
    return self.items;
}
- (SELF_TabBarItem *)currentSelectItem{
...
- (NSMutableArray<SELF_TabBarItem *> *)items{
    if (!_items) {
        _items = @[].mutableCopy;
    }
    return _items;
