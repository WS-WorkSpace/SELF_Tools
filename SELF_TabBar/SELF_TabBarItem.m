...
@implementation SELF_TabBarItem
...
- (void)itemDidLayoutBadgeLabel{
    switch (self.itemModel.itemBadgeStyle) {
        case SELF_TabBarItemBadgeStyleTopRight:{   // 右上方 默认
...
//shuang
#define iconImgView_Proportion 0.75
#define titleLabel_Proportion 0.4
- (void)itemDidLayoutControl{
    ...
    switch (self.itemModel.itemLayoutStyle) {
        case SELF_TabBarItemLayoutStyleTopPictureBottomTitle:{         // 上图片下文字 使用图3 文1比
        ...
        case SELF_TabBarItemLayoutStyleBottomPictureTopTitle:{         // 下图片上文字
        ...
        case SELF_TabBarItemLayoutStyleLeftPictureRightTitle:{         // 左图片右文字
        ...
        case SELF_TabBarItemLayoutStyleRightPictureLeftTitle:{         // 右图片左文字
            // 左右布局要重新计算图片宽度和文本高度 比例要按照相反的1：3计算
            iconImgFrame.size.width = self.frame.size.width * titleLabel_Proportion ;
            titleFrame.size.height = self.frame.size.height - self.itemModel.componentMargin.top - self.itemModel.componentMargin.bottom;
            titleFrame.size.width = self.frame.size.width - // 图片的右下终点坐标 + 边距 + 组件右边距极限
        ...
        case SELF_TabBarItemLayoutStylePicture:{                       // 单图片占满全部
        ...
        case SELF_TabBarItemLayoutStyleTitle:{                         // 单标题占满全部
    ...
    //shuang
//    self.icomImgView.backgroundColor = [UIColor yellowColor];
...
- (void)startStrringConfigAnimation{
    ...
    switch (self.itemModel.interactionEffectStyle) {
        case SELF_TabBarInteractionEffectStyleNone:{ // 无
        }break;
        case SELF_TabBarInteractionEffectStyleSpring:{ // 放大放小
        ...
        case SELF_TabBarInteractionEffectStyleShake:{ // 摇动
            animation.keyPath = @"transform.rotation";
            CGFloat angle = M_PI_4 / 10;
            animation.values = @[@(-angle), @(angle), @(-angle)];
            animation.duration = 0.2f;
        }break;
        case SELF_TabBarInteractionEffectStyleAlpha:{ // 透明
            animation.keyPath = @"opacity";
            animation.values = @[@1.0,@0.7,@0.5,@0.7,@1.0];
            animation.duration = 0.6;
        }break;
        case SELF_TabBarInteractionEffectStyleCustom:{ // 自定义动画
...
#pragma mark - SET/GET
- (void)setItemModel:(SELF_TabBarConfigModel *)itemModel{
...
- (void)setIsSelect:(BOOL)isSelect{
    ...
    //shuang
    self.titleLabel.text = _isSelect ? @"" : self.title;
    [self.titleLabel setUpRedLineIsSelected:_isSelect];
}
...
#pragma mark - 懒加载
- (SELF_TabBarBadge *)badgeLabel{
    if (!_badgeLabel) {
        _badgeLabel = [SELF_TabBarBadge new];
...
@implementation SELF_TabBarConfigModel
...
@implementation UILabel (redLine)
...
//    CGFloat w = 27;
    ...
    CGFloat h = 4;
    if (isSelect){
        UIView *line = [[UIView alloc]initWithFrame:CGRectMake(28.5, 6.5, w, h)];
